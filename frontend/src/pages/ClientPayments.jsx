...
export default function ClientPayments() {
  ...
  useEffect(() => {
    localStorage.setItem('clientPayments', JSON.stringify(records));
  }, [records]);
  ...
  const filteredRecords = records.filter(r => 
    ...
    r.invoiceRef.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const getMethodBadge = (method) => {
    switch(method) {
      case 'Bank Transfer': return <span className="px-2.5 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full text-xs font-bold tracking-wider">Bank Transfer</span>;
      case 'UPI': return <span className="px-2.5 py-1 bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 rounded-full text-xs font-bold tracking-wider">UPI</span>;
      case 'Cheque': return <span className="px-2.5 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full text-xs font-bold tracking-wider">Cheque</span>;
      case 'Cash': return <span className="px-2.5 py-1 bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400 rounded-full text-xs font-bold tracking-wider">Cash</span>;
      default: return <span className="px-2.5 py-1 bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400 rounded-full text-xs font-bold tracking-wider">{method}</span>;
    }
  };
    ...
    <div className="flex-1 space-y-6 p-2 sm:p-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 max-w-xl">
            Track and log all incoming payments received from your clients.
          </p>
        </div>
        <button 
      ...
      <div className="bg-white dark:bg-dark-900 p-4 rounded-[16px] border border-slate-200 dark:border-dark-800 flex flex-col sm:flex-row gap-4 justify-between">
        <div className="relative w-full sm:w-96">
          ...
          <input 
            type="text"
            placeholder="Search by client, receipt or invoice ref..."
            value={searchQuery}
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-white dark:bg-dark-900 rounded-[20px] border border-slate-200 dark:border-dark-800 p-12 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No payments found</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-6">
            {searchQuery ? "No records match your search criteria." : "You haven't logged any payments yet. Click the button above to log one."}
          </p>
          {!searchQuery && (
            <button 
              onClick={openAddModal}
            ...
            >
              <Plus size={16} /> Log your first payment
            </button>
          )}
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-dark-800/50 border-b border-slate-200 dark:border-dark-700">
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Receipt / Date</th>
                  ...
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Amount Received</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Method & Ref</th>
                ...
                {filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50/80 dark:hover:bg-dark-800/50 transition-colors">
                    <td className="py-4 px-6">
                      <div className="font-semibold text-slate-800 dark:text-white">{record.receiptNumber}</div>
                    ...
                    <td className="py-4 px-6">
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1">
                         INV: {record.invoiceRef || 'N/A'}
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <div className="font-semibold text-violet-600 dark:text-violet-400 text-lg">
                        +₹{parseFloat(record.amount || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <div className="mb-1">{getMethodBadge(record.paymentMethod)}</div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-dark-900 rounded-[24px] w-full max-w-3xl shadow-2xl border border-slate-200 dark:border-dark-800 flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-start justify-between p-6 pb-2 shrink-0">
              <div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">
                  {editingId ? 'Edit Payment' : 'Log Payment Received'}
                </h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  {editingId ? 'Update the details for this payment record.' : 'Fill in the details to record an incoming payment from a client.'}
                </p>
              </div>
              <button 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                    ...
                    <span>Related Invoice (Optional)</span>
                  </label>
                  <input 
                    type="text" 
                    name="invoiceRef"
            ...
            <div className="p-6 pt-2 shrink-0 flex justify-end gap-3">
              ...
              >
                <Check size={18} />
                <span>{editingId ? 'Update Payment' : 'Save Payment'}</span>
              </button>
            </div>
