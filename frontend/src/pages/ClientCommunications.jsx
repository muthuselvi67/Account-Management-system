...
export default function ClientCommunications() {
  const [records, setRecords] = useState(() => {
    const saved = localStorage.getItem('clientCommunications');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('clientCommunications', JSON.stringify(records));
  }, [records]);
  ...
  return (
    <div className="flex-1 space-y-6 p-2 sm:p-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 max-w-xl">
            Track and manage all incoming communications, enquiries, and support requests from your clients.
          </p>
        </div>
        <button 
          onClick={openAddModal}
          className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-xl font-medium flex items-center gap-2 transition-colors shadow-sm shadow-purple-600/20"
        >
          <Plus size={20} />
      ...
      <div className="bg-white dark:bg-dark-900 p-4 rounded-[16px] border border-slate-200 dark:border-dark-800 flex flex-col sm:flex-row gap-4 justify-between">
        <div className="relative w-full sm:w-96">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-dark-800 border border-slate-200 dark:border-dark-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 dark:text-white"
          />
        </div>
      </div>
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-white dark:bg-dark-900 rounded-[20px] border border-slate-200 dark:border-dark-800 p-12 text-center">
          <div className="w-20 h-20 bg-purple-50 dark:bg-purple-900/20 rounded-full flex items-center justify-center mx-auto mb-5 text-purple-500">
            <MessageSquare size={36} />
          </div>
          <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No communications found</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-6">
            {searchQuery ? "No records match your search criteria." : "You haven't logged any client communications yet. Click the button above to add one."}
          </p>
          {!searchQuery && (
            <button 
              onClick={openAddModal}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-dark-800">
                {filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50 dark:hover:bg-dark-800/50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className="font-medium text-slate-800 dark:text-white">{record.clientName}</span>
                    </td>
                    <td className="py-4 px-6">
          ...
          <div className="bg-white dark:bg-dark-900 rounded-[24px] w-full max-w-3xl shadow-2xl border border-slate-200 dark:border-dark-800 flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-start justify-between p-6 pb-2 shrink-0">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  {editingId ? 'Update the details for this communication record.' : 'Fill in the details to log a new client communication.'}
                </p>
              </div>
              <button 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    required
                    className="w-full px-4 py-2.5 bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 dark:text-white"
                  />
                </div>
                ...
                <div className="sm:col-span-2 space-y-1.5 mt-2">
                  ...
                  <textarea 
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-2.5 bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 dark:text-white resize-none"
                  ></textarea>
                </div>
            ...
            <div className="p-6 pt-2 shrink-0 flex justify-end gap-3">
              ...
              <button 
                ...
                className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-2.5 rounded-xl font-medium transition-colors shadow-sm shadow-purple-600/20"
              >
                <Check size={18} />
