...
export default function VendorInvoices() {
  ...
  useEffect(() => {
    localStorage.setItem('vendorInvoices', JSON.stringify(records));
  }, [records]);
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'Partially Paid': return <span className="px-2.5 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full text-xs font-bold uppercase tracking-wider">Partially Paid</span>;
    ...
    <div className="flex-1 space-y-6 p-2 sm:p-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 max-w-xl">
            Manage purchase invoices received from vendors and track payment statuses.
          </p>
        </div>
        <button 
      ...
      <div className="bg-white dark:bg-dark-900 p-4 rounded-[16px] border border-slate-200 dark:border-dark-800 flex flex-col sm:flex-row gap-4 justify-between">
        <div className="relative w-full sm:w-96">
          ...
          <input 
            type="text"
            placeholder="Search invoices by vendor, ID, or ref..."
            value={searchQuery}
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-white dark:bg-dark-900 rounded-[20px] border border-slate-200 dark:border-dark-800 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-6">
            {searchQuery ? "No invoices match your search criteria." : "There are currently no records for Purchase Invoices. Click the button above to log one."}
          </p>
          {!searchQuery && (
            <button 
              onClick={openAddModal}
            ...
            >
              <Plus size={16} /> Log your first invoice
            </button>
          )}
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-dark-800/50 border-b border-slate-200 dark:border-dark-700">
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Invoice ID / Date</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Vendor & Ref</th>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-dark-900 rounded-[24px] w-full max-w-3xl shadow-2xl border border-slate-200 dark:border-dark-800 flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-start justify-between p-6 pb-2 shrink-0">
              <div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">
                  {editingId ? 'Edit Purchase Invoice' : 'Log Purchase Invoice'}
                </h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  {editingId ? 'Update the details for this invoice.' : 'Log a new invoice received from a vendor.'}
                </p>
              </div>
              <button 
            ...
            <div className="p-6 pt-2 shrink-0 flex justify-end gap-3">
              ...
              >
                <Check size={18} />
                <span>{editingId ? 'Update Invoice' : 'Log Invoice'}</span>
              </button>
            </div>
