...
import { CreditCard, Plus, Search, Calendar, Hash, Building2, FileCheck, DollarSign, Activity, AlignLeft, Check, Eye, Trash2, X } from 'lucide-react';

export default function VendorPayments() {
  const [records, setRecords] = useState(() => {
    const saved = localStorage.getItem('vendorPayments');
    if (saved) {
  ...
  const [formData, setFormData] = useState({
    ...
    paymentId: `VPAY-${Math.floor(Math.random() * 9000) + 1000}`,
    vendorName: '',
    invoiceReference: '',
    amount: '',
  ...
  useEffect(() => {
    localStorage.setItem('vendorPayments', JSON.stringify(records));
  }, [records]);
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'Completed': return <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 rounded-full text-xs font-bold uppercase tracking-wider">Completed</span>;
      case 'Processing': return <span className="px-2.5 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full text-xs font-bold uppercase tracking-wider">Processing</span>;
      case 'Failed': return <span className="px-2.5 py-1 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded-full text-xs font-bold uppercase tracking-wider">Failed</span>;
    ...
    <div className="flex-1 space-y-6 p-2 sm:p-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 max-w-xl">
            Track, manage, and record all outgoing payments to vendors.
          </p>
        </div>
        <button 
      ...
      <div className="bg-white dark:bg-dark-900 p-4 rounded-[16px] border border-slate-200 dark:border-dark-800 flex flex-col sm:flex-row gap-4 justify-between">
        <div className="relative w-full sm:w-96">
          ...
          <input 
            type="text"
            placeholder="Search by vendor, payment ID or invoice ref..."
            value={searchQuery}
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-white dark:bg-dark-900 rounded-[20px] border border-slate-200 dark:border-dark-800 p-12 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No payment records found</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-6">
            {searchQuery ? "No records match your search criteria." : "You haven't logged any vendor payments yet. Click the button above to add one."}
          </p>
          {!searchQuery && (
            <button 
              onClick={openAddModal}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-dark-800/50 border-b border-slate-200 dark:border-dark-700">
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Payment ID / Date</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Vendor & Invoice Ref</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Amount Paid</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status / Method</th>
                ...
                {filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50/80 dark:hover:bg-dark-800/50 transition-colors">
                    <td className="py-4 px-6">
                      <div className="font-semibold text-slate-800 dark:text-white">{record.paymentId}</div>
                    ...
                    <td className="py-4 px-6">
                      <div className="mb-1.5">{getStatusBadge(record.status)}</div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-dark-900 rounded-[24px] w-full max-w-3xl shadow-2xl border border-slate-200 dark:border-dark-800 flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-start justify-between p-6 pb-2 shrink-0">
              <div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">
                  {editingId ? 'Edit Payment Record' : 'Log Vendor Payment'}
                </h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  {editingId ? 'Update the details for this payment.' : 'Log a new payment sent to a vendor.'}
                </p>
              </div>
              <button 
            ...
            <div className="p-6 pt-2 shrink-0 flex justify-end gap-3">
              ...
              >
                <Check size={18} />
                <span>{editingId ? 'Update Payment' : 'Log Payment'}</span>
              </button>
            </div>
