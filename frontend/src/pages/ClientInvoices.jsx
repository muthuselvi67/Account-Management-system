...
import { FileCheck, X, Plus, Search, Calendar, Hash, Building2, Briefcase, DollarSign, Percent, Clock, Activity, AlignLeft, Check, Eye, Trash2, ArrowLeft } from 'lucide-react';
...
export default function ClientInvoices() {
  ...
  useEffect(() => {
    localStorage.setItem('clientInvoices', JSON.stringify(records));
  }, [records]);
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'Unpaid': return <span className="px-2.5 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full text-xs font-bold uppercase tracking-wider">Unpaid</span>;
      case 'Partially Paid': return <span className="px-2.5 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full text-xs font-bold uppercase tracking-wider">Partial</span>;
      case 'Paid': return <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 rounded-full text-xs font-bold uppercase tracking-wider">Paid</span>;
    ...
    <div className="flex-1 space-y-6 p-2 sm:p-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white flex items-center gap-3">
            <div className="p-2 bg-violet-100 dark:bg-violet-900/30 rounded-lg">
              <FileCheck className="text-violet-600 dark:text-violet-400" size={28} />
            </div>
            Sales Invoices (GST)
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 max-w-xl">
            Create, track, and manage all GST invoices issued to your clients.
          </p>
        </div>
        <button 
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-white dark:bg-dark-900 rounded-[20px] border border-slate-200 dark:border-dark-800 p-12 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No invoices found</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-6">
            {searchQuery ? "No records match your search criteria." : "You haven't issued any sales invoices yet. Click the button above to generate one."}
          </p>
          {!searchQuery && (
            <button 
              onClick={openAddModal}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-dark-800/50 border-b border-slate-200 dark:border-dark-700">
                  ...
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Client & PO Ref</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Amount</th>
                  <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status & Due</th>
                ...
                {filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50/80 dark:hover:bg-dark-800/50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1">
                         PO: {record.referencePo || 'N/A'}
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <div className="font-semibold text-slate-800 dark:text-white">
                        ₹{parseFloat(record.total || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                      </div>
                    ...
                    <td className="py-4 px-6">
                      ...
                      {record.dueDate && (
                        <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 flex items-center gap-1">
                          <Clock size={12} /> Due: {new Date(record.dueDate).toLocaleDateString()}
                        </div>
                      )}
                    </td>
            ...
            <div className="flex items-start justify-between p-6 pb-2 shrink-0">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  {editingId ? 'Update the details for this invoice.' : 'Fill in the details to generate a new GST invoice for a client.'}
                </p>
              </div>
              <button 
